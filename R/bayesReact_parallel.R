...
#' @param samples_per_partition number of samples to be included in each parallel job. Default is 20. Smaller values will result in more jobs that individually will run faster, however, more jobs needs to be allocated resources by the Slurm queuing system.
...
bayesReact_parallel <- function(lst_data, out_path, out_name = "motif_activity", save_as_bigmat = F, account, samples_per_partition = 20,
  ...
  if (!(model %in% c("bayesReact", "BF", "bayesReact_2param"))) {
    cat("Error: \'model\' must be either \"bayesReact\", \"BF\", or \"bayesReact_2param\"\n", file = logs_con, append = T)
    stop("model must be either \"bayesReact\", \"BF\", or \"bayesReact_2param\"", call. = F)
  }
  ...
  if (model == "BF" & requireNamespace("bridgesampling", quietly = TRUE) == F) {
    cat("Error: The bridgesampling package is required for model = \"BF\". Please install the package and try again.\n", file = logs_con, append = T)
    stop("The bridgesampling package is required for model = \"BF\". Please install the package and try again.", call. = F)
  }
  ...
  if(save_as_bigmat == T){
    if(requireNamespace("bigmemory", quietly = TRUE) == F) {
      cat("Error: The bigmemory package is required for save_as_bigmat = T. Please install the package and try again.", file = logs_con, append = T)
      stop("The bigmemory package is required for save_as_bigmat = T. Please install the package and try again.", call. = F)
    }
  }
